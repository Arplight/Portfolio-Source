...
function WorkComponent(Props) {
  return (
    <div className="work flex items-center flex-col xl:flex-row gap-y-10 xl:gap-y-0 gap-x-0 xl:gap-x-8 ">
          ...
          <div className="buttons mt-10 hidden xl:flex ">
            <a
              ...
              className={`main-button button-light button-dark mr-5 ${
                !Props.withMultiLinks && "w-full"
              } `}
              rel="noopener noreferrer"
            >
              {Props.projectType === "full" ? "WEBSITE" : "DISCOVER"}
            </a>
            {Props.withMultiLinks && (
              <a
                ...
                rel="noopener noreferrer"
              >
                {Props.projectType === "full" ? "DASHBOARD" : "VIEW CODE"}
              </a>
            )}
          </div>
        </Fade>
      ...
      <Fade key={Math.random()} delay={800}>
        <div className="buttons mt-5 flex flex-col w-full xl:hidden gap-4">
          <a
            href={Props.projectDemo}
            target="_blank"
            className="main-button button-light button-dark mr-5  w-full"
            rel="noopener noreferrer"
          >
            {Props.projectType === "full" ? "WEBSITE" : "DISCOVER"}
          </a>
          {Props.withMultiLinks && (
            <a
              href={Props.projecCode}
              target="_blank"
              className="main-button button-light button-dark w-full"
              rel="noopener noreferrer"
            >
              {Props.projectType === "full" ? "DASHBOARD" : "VIEW CODE"}
            </a>
